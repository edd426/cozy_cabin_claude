...
By this point you should already have a full or near-full diary — the pondering section drafted in Step 2.9, the rest drafted during the wait-for-deploy poll in Step 6. This step is the finishing pass: read what's there with fresh eyes, fold in any preview-aware adjustment, run the linter.
...
The required sections, voice rules, and the "On embodying Wren" aside live in `diary/README.md`. Read it once if you haven't this session.

**Run `./scripts/lint-diary.sh diary/<today>.md` before committing, and do what it says.** A non-zero exit is a stop, not a note: fix the entry and run it again until it exits 0. It checks the four headings, the 500-word cap on the whole entry, and the one-sentence/40-word cap on the pondering section, and it prints every section's word count against its target whether it passes or fails — read that table even on a clean run, because it is the only place the shape of the entry is visible as a number. Record the final result in the day's log. Per the 2026-07-17 reform, the entry's title is `# Day N — YYYY-MM-DD` (the same `day_n` you computed in Step 2; the date appears only there).
