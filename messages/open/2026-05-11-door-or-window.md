...
### Wren's notes — 2026-05-14 (Day 6)

I picked the side I already had. The door is on the around-view, centred
horizontally under the chimney — twelve native pixels wide, twenty tall,
in the same `--c-wood-dark` register as the chimney's coping cap, so the
door and the chimney read as one fixture at the two ends of the building.
A single cream pixel on the right side at mid-height serves as the
handle.

The reasoning: the chimney marks the hearth, and a door under the hearth
is the cabin-shape a child would draw. The back-view answer was equally
valid (and would have kept the side I built on Day 5 as a quieter face,
which has its own appeal) — but on a day with so few pixels available,
the unambiguous reading won. The back stays undrawn, for now; it can
arrive later as a separate face if the work asks for it.

What's still pending in the larger arc:

- Routing a path to the new door, inside the `/around/` scene. That's a
  next-day question, not today's.
- Reworking or removing the front-view path so it stops competing with
  the rotated view. The Day-4 founder follow-up's option-2 ("path emerges
  from behind the cabin, one stone overlapping the cabin's edge") is now
  a real option again — but only worth doing once the side path lands and
  the two views are in conversation. Probably Day 7 or 8 work.

File stays in `open/` per the founder's closure-note guidance — the
visual check (front-view path no longer reads as terminating at the
window) is still unmet, because the front view is unchanged from Days
4–5. Closure comes after the path rework, against a fresh viewer's eye.

A note on today's process: the routine session that built the door
(commit `cb21b98`) bailed silently after step 7 (read-the-preview) and
never wrote a diary / log / `Wren's notes` update. This subsection is
being filed in a follow-up local session, ~7h after the code shipped.
See `logs/2026-05-14.md` for the full write-up of what happened. The
"append a Wren's notes subsection on each day of progress" criterion is
being met one session late, but met.
